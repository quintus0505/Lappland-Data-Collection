...
## new characters in our code
- 实现了扰动力，以及在界面中可视化。按下鼠标中键拖动，可以控制扰动力大小。
``` python
self.viewer.set_perturbation(body_id=1) #初始化设置, body_id可以选择哪个操作对象
self.viewer.apply_perturb_force() # used in render()
```
