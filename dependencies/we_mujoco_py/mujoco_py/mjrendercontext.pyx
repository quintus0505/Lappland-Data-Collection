...
# //////////////////////////////////////////////////////////////////////
# the following functions are used to add perturbation force in the viewer
    def zero_perturb_data(self):
        """
        clear mjData.xfrc_applied for the remaining bodies,
        in case the selected body changed and some perturbation force was left over from a previous time step
        """
        mju_zero(self._data_ptr.xfrc_applied, 6* self._model_ptr.nbody)

    def init_perturb(self):
        """
        set the reference pose equal to the selected body pose at the onset of perturbation, so as to avoid jumps
        """
        mjv_initPerturb(self._model_ptr, self._data_ptr, &self._scn, &self._pert)

    def move_perturb(self, int action, double reldx, double reldy):
        """
        a mouse hook for controlling the reference pose with the mouse.
        """
        mjv_movePerturb(self._model_ptr, self._data_ptr, action, reldx, reldy, &self._scn, &self._pert)

    def set_pert_enable(self, active, bodyID=1, skinID=1):
        self._pert.select= bodyID
        self._pert.skinselect = skinID
        self._pert.active = active

    def pert_enable(self):
        return self._pert.active

    def mj_apply_perturb_force(self):
        """
        apply forces and torques to the body.
        writes the external perturbation force to mjData.xfrc_applied for the selected body
        """
