...
class MjViewerBasic(cymj.MjRenderContextWindow):
    ...
    def __init__(self, sim):
        ...
        # used for perturbation force visualization
        self.use_perturbation = False
        self.pert_body_id = 1 # the body to manipulate
        self.pert_skin_id = 1 # the skin-ID related to body, seem to be useless?
        self._button_middle_pressed = False
        self._button_middle_released = False
        self._enter_pert = False
    ...
    def set_perturbation(self, body_id=1, skin_id=1):
        """
        :param body_id: the perturbated body, the perturbated location also can be customed in the MjRenderContext.pert
        :param skin_id: may be useless
        :return:
        """
        self.use_perturbation = True
        self.pert_body_id = body_id
        self.pert_skin_id = skin_id
    ...
    def _cursor_pos_callback(self, window, xpos, ypos):
        if not (self._button_left_pressed or self._button_right_pressed or self._button_middle_pressed):
            return
        ...
        elif self._button_middle_pressed:
            action = const.MOUSE_MOVE_H if mod_shift else const.MOUSE_MOVE_V
        else:
        ...
        if self._enter_pert and self.use_perturbation:
            with self._gui_lock:
    ...
    def apply_perturb_force(self):
        if self.use_perturbation:
            self.zero_perturb_data()
            self.mj_apply_perturb_force()
    ...
    def _mouse_button_callback(self, window, button, act, mods):
        ...
        if self.use_perturbation:
            ...
            self._button_middle_released = (glfw.get_mouse_button(window, glfw.MOUSE_BUTTON_MIDDLE)== glfw.RELEASE)
            newperturb = 0
            if self._button_middle_pressed:
                self._button_middle_released = False
                newperturb = 1
                if newperturb and not self.pert_enable():
                    self.init_perturb()
                self.set_pert_enable(newperturb, self.pert_body_id, self.pert_skin_id)
                self._enter_pert = True
            if self._button_middle_released:
                self._button_middle_pressed = False
                self.set_pert_enable(0, self.pert_body_id, self.pert_skin_id)
                self._enter_pert = False
...
class MjViewer(MjViewerBasic):
    ...
    def render(self):
        ...
        def render_inner_loop(self):
            ...
            if not self._hide_overlay:
                ...
                self._create_full_overlay() #TODO:
            super().render()
    ...
    def _create_full_overlay(self): #TODO:
        if self._render_every_frame:
