...
def mcs_callback(RigidBodyPose):
    ...
    # i_hehe+=0.01
    # print(i_hehe)
    # x_input = 180 * math.pi / 180.0 + i_hehe
    # if x_input >=180.0* math.pi / 180.0:
    #     x_input-=360.0* math.pi / 180.0
    # if x_input <=-180.0* math.pi / 180.0:
    #     x_input+=360.0* math.pi / 180.0
    # euler_test = [90*math.pi/180.0+i_hehe, 0.*math.pi/180.0, 160*math.pi/180.0]
    # quat_test = euler2quat(euler_test)
    # v[3:7] = quat_test[:]

    # print(quat2euler(abs_quat)*180.0/3.1415926)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while not goal_done:
            ...
            action = np.zeros(7, dtype=np.float)
            for i in range(7):
                action[i] = ee_pos[i]
            # print(action)
            ...
            # action[9]=-0.5
            # action[13] = 1.57
            hand_ctrl = np.zeros(13, dtype=np.float)
            for j in range(13):
                hand_ctrl[j] = finger_angles[j]*math.pi/180.0
            ...
            obs, reward, done, info = env.mcs_demonstration(action[0:3], action[3:7], hand_ctrl)
