...
class MiniCheetahEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):

        self.viewer_initialized = False
        self.use_perturbation = True # whether use perturbation force in viewer

        curr_dir = os.path.dirname(os.path.abspath(__file__))
        mujoco_env.MujocoEnv.__init__(self, curr_dir+'/../assets/mini_cheetah/mini_cheetah.xml', 2)
    ...
    def mj_viewer_setup(self, tracking=True):
        ...
        self.viewer.set_perturbation(body_id=1)

    def render(self, mode='human'):
        if self.viewer_initialized:
            self.viewer.apply_perturb_force()
        self.mj_render()
