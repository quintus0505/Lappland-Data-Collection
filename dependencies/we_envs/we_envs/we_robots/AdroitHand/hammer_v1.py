...
class HammerEnvV1(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        ...
        self.sim.model.actuator_gainprm[self.sim.model.actuator_name2id('A_WRJ1'):self.sim.model.actuator_name2id('A_WRJ0')+1, 0:3] = np.array([10, 0, 0])
        self.sim.model.actuator_gainprm[self.sim.model.actuator_name2id('A_FFJ3'):self.sim.model.actuator_name2id('A_THJ0')+1, 0:3] = np.array([1, 0, 0])
        self.sim.model.actuator_biasprm[self.sim.model.actuator_name2id('A_WRJ1'):self.sim.model.actuator_name2id('A_WRJ0')+1, 0:3] = np.array([0, -10, 0])
        self.sim.model.actuator_biasprm[self.sim.model.actuator_name2id('A_FFJ3'):self.sim.model.actuator_name2id('A_THJ0')+1, 0:3] = np.array([0, -1, 0])
    ...
    def step(self, a):
        ...
        # get to hammer
        reward = - 0.1 * np.linalg.norm(palm_pos - terminal_palm_pos)
        reward_tb[0] = reward
        # velocity penalty
        reward -= 1e-2 * np.linalg.norm(self.data.qvel.ravel())        

        reward -= 1.0*np.linalg.norm(obj_pos-self.init_state['init_obj_pos'])
        reward_tb[1] = -1.0*np.linalg.norm(obj_pos-self.init_state['init_obj_pos'])

        goal_achieved = True if np.linalg.norm(palm_pos -  terminal_palm_pos) < 0.05 and np.linalg.norm(obj_pos-self.init_state['init_obj_pos'])<0.05 else False
        if goal_achieved:
            reward+=10.0
            reward_tb[2] = 10.0
    ...
    def get_env_state(self):
        ...
        return dict(qpos=qpos, qvel=qvel, board_pos=board_pos, target_pos=target_pos, obj_pos=obj_pos,palm_pos=palm_pos,tool_pos=tool_pos,goal_pos=goal_pos)
