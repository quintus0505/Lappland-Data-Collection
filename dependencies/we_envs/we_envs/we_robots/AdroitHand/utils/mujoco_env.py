...
class MujocoEnv(gym.Env):
    ...
    def get_env_dim(self):
        ...
        low = -high
        self.observation_space = spaces.Box(low, high)
        return self.obs_dim, self.action_space.shape
    ...
    # -----------------------------
    def close(self):
        # https://www.programcreek.com/python/example/124902/glfw.destroy_window
    ...
    def render_text(self, name, text, location='top_left', interval=8):
        from mujoco_py.generated import const
        switch = {'top_right':const.GRID_TOPRIGHT, 'top_left':const.GRID_TOPLEFT,
                  'bottom_left': const.GRID_BOTTOMLEFT, 'bottom_right':const. GRID_BOTTOMRIGHT}
        for _ in range(interval):
            self.viewer.add_overlay(switch.get(location, const.GRID_TOPRIGHT),str(''), str(''))
        self.viewer.add_overlay(switch.get(location, const.GRID_TOPRIGHT),str(name), str(text))
    ...
    def visualize_policy(self, policy, horizon=1000, num_episodes=1, mode='exploration'):
        ...
        for ep in range(num_episodes):
            ...
            while t < horizon and d is False:
                ...
                try:
                    d = info['goal_achieved']
                except:
                    print('info[goal_achieved] is not included in environment!')
                t = t+1
        self.mujoco_render_frames = False
