...
class RelocateEnvV2(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
    ...
    def step(self, a):
        ...
        # if obj_pos[2] > 0.04 and obj_pos[2] < 0.08:    # if object off the table
        #     if np.linalg.norm(palm_pos-obj_pos)<0.055:
        #         reward += 10.0*obj_pos[2]                                            # bonus for lifting the object
        #         reward_tb[3] = 10.0*obj_pos[2]
        #
        # if obj_pos[2] > 0.10:
        #     reward -= 2.0*obj_pos[2]
        ...
        goal_achieved = True if (np.linalg.norm(palm_pos-obj_pos) < 0.05 and np.linalg.norm(palm_pos - terminal_palm_pos)<0.05 and np.linalg.norm(obj_pos - terminal_obj_pos)<0.05 )else False
        if(goal_achieved):
            reward += 1.0
