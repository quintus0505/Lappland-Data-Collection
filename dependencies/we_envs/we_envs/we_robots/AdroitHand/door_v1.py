...
class DoorEnvV1(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
    ...
    def step(self, a):
        ...
        if np.linalg.norm(palm_pos-terminal_palm_pos)<dis_constrain-0.01:
            reward -= 0.4/np.linalg.norm(palm_pos-terminal_palm_pos)
        ...
        goal_achieved = True if np.linalg.norm(terminal_palm_pos-palm_pos)<dis_constrain and latch_pos<0.05 else False
        if goal_achieved:
            reward += 10.0
            reward_tb[3] = 10.0
