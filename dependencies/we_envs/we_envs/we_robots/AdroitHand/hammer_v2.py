...
class HammerEnvV2(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
    ...
    def step(self, a):
        ...
        # velocity penalty
        reward -= 1e-2 * np.linalg.norm(self.data.qvel.ravel())
        if np.linalg.norm((tool_pos - target_pos)) < 0.2:
            reward -=  0.2*np.linalg.norm(self.data.qvel.ravel())

        # take hammer head to nail
        reward -= np.linalg.norm((tool_pos - target_pos))
        reward_tb[1] = -np.linalg.norm((tool_pos - target_pos))

        reward -= 5.0*np.linalg.norm(target_pos-self.init_state['init_target_pos'])

        goal_achieved = True if np.linalg.norm(palm_pos - obj_pos) < 0.05\
            and np.linalg.norm((tool_pos - target_pos)) <0.06 and \
                                np.linalg.norm(target_pos-self.init_state['init_target_pos'])<0.05 else False
        if goal_achieved:
            reward += 20.0
            reward_tb[2] = 20.0
